...
impl OscillatorType {
    ...
    pub(crate) fn build_lut(
        ...
        let raw_lut: Vec<f32>  = match self {
            OscillatorType::Sine => {
...
impl Oscillator {
    ...
    pub(crate) fn new(lut: RefCell<Vec<Vector2>>, sample_rate: usize) -> Self {
        Self { lut, sample_rate }
    }

    /// Fill the output buffer with generated sound.
    pub(crate) fn generate(&mut self, output: &mut [Vector2], offset: usize) {
