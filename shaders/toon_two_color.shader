...
uniform vec4 albedo1 : hint_color = vec4(1.0);
uniform vec4 albedo2 : hint_color = vec4(1.0);
...
void fragment() {
	if (UV.x > 0.5) {
		ALBEDO = albedo1.rgb * texture(texture_albedo, UV).rgb;
	} else {
		ALBEDO = albedo2.rgb * texture(texture_albedo, UV).rgb;
	}
